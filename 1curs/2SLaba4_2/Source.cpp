...
void enter_new()
{
	...
	if (strnum < size)
	{
		...
		cout << "Адресс " << endl;
		cin >> list_of_zap[strnum].tel;
		cout << "Телефон " << endl;
		cin >> list_of_zap[strnum].mesto;
		cout << "Место работы или учебы " << endl;
		cin >> list_of_zap[strnum].adres;
		cout << "Должность " << endl;
		cin >> list_of_zap[strnum].status;
		strnum++;
	}
	else
...
void out()
{
	...
	if (sw == 1)
	{
		...
		cout << endl << "ФИО: " << list_of_zap[tabl.coutnomer - 1].fio << endl;
		cout << "Дата рождения: " << list_of_zap[tabl.coutnomer - 1].data << endl;
		cout << "Адресс: " << list_of_zap[tabl.coutnomer - 1].adres << endl;
		cout << "Телефон: " << list_of_zap[tabl.coutnomer - 1].tel << endl;
		cout << "Место работы или учебы: " << list_of_zap[tabl.coutnomer - 1].mesto << endl;
		cout << "Должность: " << list_of_zap[tabl.coutnomer - 1].status << endl;
		cout << "-------------------------------------------------------------------------------------";
	}
	if (sw == 2)
	{
...
void find(char fio[16])
{
	...
	for (; tabl.coutnomer < str_len; tabl.coutnomer++)
		...
		if (strcmp(fio, list_of_zap[tabl.coutnomer].fio) == 0)
		{
			...
			cout << "-------------------------------------------------------------------------------------";
			cout << endl << "ФИО: " << list_of_zap[tabl.coutnomer].fio << endl;
			cout << "Дата рождения: " << list_of_zap[tabl.coutnomer].data << endl;
			cout << "Адресс: " << list_of_zap[tabl.coutnomer].adres << endl;
			cout << "Телефон: " << list_of_zap[tabl.coutnomer].tel << endl;
			cout << "Место работы или учебы: " << list_of_zap[tabl.coutnomer].mesto << endl;
			cout << "Должность: " << list_of_zap[tabl.coutnomer].status << endl;
			cout << "-------------------------------------------------------------------------------------" << endl;
		}
	if (flag == false)
